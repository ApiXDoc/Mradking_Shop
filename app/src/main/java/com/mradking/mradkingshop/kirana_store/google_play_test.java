...
import com.mradking.mradkingshop.kirana_store.custmer.intface.Pay_Call;
import com.mradking.mradkingshop.kirana_store.custmer.utility.UpiX;

import java.time.Duration;
import java.util.ArrayList;
public class google_play_test extends AppCompatActivity {
    EditText amount, note, name, upivirtualid;
    Button send;
    String TAG ="main";
    final int UPI_PAYMENT = 0;
    Activity activity=google_play_test.this;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.google_pay_test);
        ...
        send.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (TextUtils.isEmpty(name.getText().toString().trim())){
                    Toast.makeText(activity," Name is invalid", Toast.LENGTH_SHORT).show();
                }else if (TextUtils.isEmpty(upivirtualid.getText().toString().trim())){
                    Toast.makeText(activity," UPI ID is invalid", Toast.LENGTH_SHORT).show();
                }else if (TextUtils.isEmpty(note.getText().toString().trim())){
                    Toast.makeText(activity," Note is invalid", Toast.LENGTH_SHORT).show();
    ...
    private void upiPaymentDataOperation(ArrayList<String> data) {
        if (isConnectionAvailable(activity)) {
            ...
            else if (status.equals("failure")) { // Add this condition to handle transaction failure.
                Toast.makeText(activity, "Transaction failed. Please try again", Toast.LENGTH_SHORT).show();
                Log.e("UPI", "failed payment: "+approvalRefNo);
            }
            else if (status.equals("submitted") || status.equals("pending")) {
                //Code to handle pending transaction here.
                Toast.makeText(activity, "Transaction pending. Please check your UPI app for more details.", Toast.LENGTH_SHORT).show();
                Log.e("UPI", "pending payment: "+approvalRefNo);
            }
